// The same small in-memory user service, used here by the cucumber-js example.
// Plain Node http, no dependencies. Exported so the cucumber hooks start it.
...
export function createServer() {
  ...
  return http.createServer(async (req, res) => {
    ...
    if (method === "POST" && url === "/users") {
      ...
      if (!body.name || typeof body.name !== "string") return send(res, 400, { error: "name is required" });
